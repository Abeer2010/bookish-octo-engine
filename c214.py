...
def lidar_data(point_cloud,lidar_point_cloud_buffer):
    matrix_representional_data = np.copy(np.frombuffer(point_cloud.raw_data, dtype=np.dtype('f4')))
    reshape_of_image = np.reshape(matrix_representional_data,(int(matrix_representional_data.shape[0] / 4), 4))
